...
import { convertCurrencyInputToBase, convertCurrencyValue, formatShortDate, incomeCategories, type CurrencyCode } from "@/lib/utils";
...
function IncomeForm({ initialValues, onSubmit, trigger, title, description }: { initialValues?: Partial<IncomeFormValues>; onSubmit: (values: IncomeValues) => Promise<void>; trigger: React.ReactNode; title: string; description: string; }) {
  const { currency } = useCurrency();
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [entryCurrency, setEntryCurrency] = useState<CurrencyCode>(currency);
  const form = useForm<IncomeFormValues, unknown, IncomeValues>({
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      category: (initialValues?.category as IncomeValues["category"]) || "Salary",
        ...
        <form className="space-y-4" onSubmit={form.handleSubmit(async (values) => {
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select value={form.watch("category")} onValueChange={(value) => form.setValue("category", value as IncomeFormValues["category"])}>
                <SelectTrigger>
...
export function IncomePageClient({ initialRecords }: { initialRecords: IncomeRecord[] }) {
  ...
  async function createRecord(values: IncomeValues) {
  ...
  async function updateRecord(id: string, values: IncomeValues) {
  ...
  const totalIncome = filtered.reduce((sum, record) => sum + Number(record.amount), 0);
    ...
    <div className="space-y-6">
      <Card className="surface-shine overflow-hidden">
        <CardContent className="flex flex-col gap-4 pt-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm text-muted-foreground">Filtered income total</p>
            <p className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-primary">{formatCurrency(totalIncome)}</p>
          </div>
          <IncomeForm
            title="Add Income"
            description="Capture a new income transaction."
            onSubmit={createRecord}
            trigger={<Button><Plus className="h-4 w-4" />Add Income</Button>}
          />
        </CardContent>
      </Card>
        ...
        <CardContent className="grid gap-3 pt-6 md:grid-cols-4">
          <Select value={categoryFilter} onValueChange={setCategoryFilter}>
            ...
            <SelectContent>
              ...
              {incomeCategories.map((category) => <SelectItem key={category} value={category}>{category}</SelectItem>)}
            </SelectContent>
          </Select>
      ...
      <div className="glass-card overflow-hidden rounded-3xl animate-rise-in">
        {paginated.length === 0 ? (
          <div className="p-10 text-center text-sm text-muted-foreground">No income records match the current filters.</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableRow key={record.id}>
                  ...
                  <TableCell>
                    <div className="flex justify-end gap-2">
                      <IncomeForm
                        title="Edit Income"
                        description="Update this income entry."
                        initialValues={{ title: record.title, amount: Number(record.amount), category: record.category as IncomeFormValues["category"], date: record.date, note: record.note || "" }}
